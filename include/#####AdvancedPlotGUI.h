...
class RootDataInspector;
//RootDataInspector* rootInspector = nullptr;
...
private:
    ...
    void ShowCSVPreview(const char* filepath);
    void LoadCSVWithSettings(const char* filepath, char delimiter, Int_t skipRows, Bool_t useHeader);
    Bool_t LoadRootFile(const char* filepath);
...
public:
    ...
    void DoClearEditor();
    void DoClearOutput();
    void DoLoadFromDrop(const char* path);
